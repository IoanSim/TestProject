(function () {
    ...
    function siteManagerEditController($scope, $http, $routeParams, $location, formHelper, notificationsService) {
        ...
        function save() {
            if (formHelper.submitForm({ scope: $scope })) {
                ...
                $http.post("backoffice/SeoToolkit/ScriptManager/Save", postModel).then(function (response) {
        ...
        function loadDefinitions() {
            ...
            $http.get("backoffice/SeoToolkit/ScriptManager/GetAllDefinitions").then(function (response) {
        ...
        function loadScript(id) {
            //TODO: Clean up
            vm.page.isLoading = true;
            $http.get("backoffice/SeoToolkit/ScriptManager/GetAllDefinitions").then(function (definitionResponse) {
                vm.definitions = definitionResponse.data;

                $http.get("backoffice/SeoToolkit/ScriptManager/Get?id=" + id).then(function (response) {
                    vm.content = {
                        id: response.data.id,
        ...
        function back() {
            $location.path("SeoToolkit/ScriptManager/list").search("id", null);;
        }
    ...
    angular.module("umbraco").controller("SeoToolkit.ScriptManager.EditController", siteManagerEditController);
})();