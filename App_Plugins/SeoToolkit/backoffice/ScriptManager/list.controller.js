(function () {
    ...
    function scriptManagerListController($scope, $http, $location, listViewHelper, notificationsService) {
        ...
        function create() {
            $location.path("SeoToolkit/ScriptManager/edit").search("id", null);;
        }

        function deleteSelection() {
            vm.loading = true;
            $http.post("backoffice/SeoToolkit/ScriptManager/Delete",
                {
        ...
        function setItems(items) {
            vm.items = items.map(function (i) {
                return {
                    ...
                    "published": true,
                    "editPath": "SeoToolkit/ScriptManager/edit?id=" + i.id
                }
            });
        }
        ...
        function init() {
            vm.loading = true;
            $http.get("backoffice/SeoToolkit/ScriptManager/GetAllScripts").then(function (response) {
                setItems(response.data);
                vm.loading = false;
            });
        }
    ...
    angular.module("umbraco").controller("SeoToolkit.ScriptManager.ListController", scriptManagerListController);
})();